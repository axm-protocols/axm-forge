...
| `axm_mcp.server.serve` | Run HTTP; unlike CLI, a missing port can use the environment. Records `shared`/`dedicated` for `/health`'s `serve_mode` and `write_contracts_enforced` |
