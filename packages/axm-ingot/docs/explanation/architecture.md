...
graph TD
    subgraph "axm_ingot.uv"
        ...
        Resolve["resolve.py — resolve_workspace, find_workspace_root"]
    end
    Resolve --> Models
```
...
The resolution logic — `resolve_workspace` and `find_workspace_root`, plus
private helpers (`_load_pyproject`, `_get_workspace_config`,
`_resolve_glob_dirs`). Pure functions over the filesystem; all TOML parsing is
defensive.
