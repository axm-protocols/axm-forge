...
| `callers.py` | Call-site detection and non-call reference extraction (dynamic dispatch patterns: dict values, list/tuple/set elements, keyword arguments, default parameters, positional arguments). Shares tree-sitter walker primitives (`is_call_node`, `update_context`, `extract_call_site`, `node_text_safe`) with `flows.py` via the internal `_call_helpers` module — exposed without underscore prefix so cross-module imports remain compliant with the no-private-cross-module-import rule | `find_callers()`, `find_callers_workspace()`, `extract_references()` |
