...
class TestModuleDottedName:
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                Path("/tmp/pkg/src/mypkg/core.py"), "mypkg.core", id="src_layout"
            ),
            pytest.param(
                Path("/tmp/pkg/mypkg/core.py"), "mypkg.core", id="flat_layout"
            ),
            pytest.param(
                Path("/tmp/pkg/src/mypkg/__init__.py"), "mypkg", id="init_file"
            ),
            pytest.param(
                Path("/tmp/pkg/src/src/__init__.py"), "src", id="package_named_src"
            ),
            pytest.param(
                Path("/tmp/pkg/src/mypkg/src/inner.py"),
                "mypkg.src.inner",
                id="nested_src_dirs",
            ),
        ],
    )
    def test_module_dotted_name(self, module_path: Path, expected: str) -> None:
        result = module_dotted_name(module_path, Path("/tmp/pkg"))
