...
def test_custom_prefix(tmp_git_repo: Path) -> None:
...
def test_branch_exists_fails(tmp_git_repo: Path) -> None:
    hook = CreateBranchHook()
    hook.execute(
        {"working_dir": str(tmp_git_repo), "session_id": "x"},
    )
...
def test_create_branch_ticket_based(tmp_git_repo: Path) -> None:
...
def test_create_branch_direct_param(tmp_git_repo: Path) -> None:
...
def test_create_branch_param_overrides_ticket(tmp_git_repo: Path) -> None:
...
def test_create_branch_session_fallback(tmp_git_repo: Path) -> None:
...
def test_missing_ticket_title_falls_back(tmp_git_repo: Path) -> None:
...
def test_empty_labels(tmp_git_repo: Path) -> None:
