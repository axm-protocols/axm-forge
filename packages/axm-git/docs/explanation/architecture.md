...
- **`gh_auth.py` and `credentials.py`** — `gh_auth_state()` classifies only observable process state from `gh auth status` as `logged_in`, `logged_out`, or `not_installed`. `GH_AUTH_CREDENTIAL` publishes that check, the status command, and the `gh auth login` recovery command through the credential catalog; it never reads a token or session file.
