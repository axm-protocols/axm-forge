...
# Derived, not hard-coded: the suite directory is ``tests_<pkg>`` (unique module
# path per package), so spelling "tests/" here would silently point nowhere.
_SUITE_DIR = pathlib.Path(__file__).resolve().parents[1].name
_TARGET = f"{_SUITE_DIR}/integration/test_identity__load_config.py"
