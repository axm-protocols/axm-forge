...
It scans ``src/axm_edit/`` for Python modules and generates a
``::: module.path`` page for each, which mkdocstrings then renders
under ``reference/api/axm_edit/``. A ``SUMMARY.md`` is emitted for the
...
``mkdocs.yml`` and the ``reference/api/axm_edit/...`` links in
``index.md``.
