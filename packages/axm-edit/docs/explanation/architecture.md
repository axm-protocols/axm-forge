...
- **Whole-line anchors with a terminal newline** — a replace anchor may include the final `\n` of every line it spans. For example, `old="beta\n"` is aligned inside `"alpha\nbeta\ngamma\n"`, and an anchor ending with the file's final newline is aligned too. The preflight computes boundaries from the exact decoded text and splits only on `\n`, so form-feed and other non-LF characters remain anchor data. Anchors that genuinely start or end mid-line still produce `ANCHOR_NOT_WHOLE_LINE`.
