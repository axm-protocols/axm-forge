...
def _tests_override(mypy: dict[str, object]) -> dict[str, object] | None:
    ...
    for override in overrides:
        ...
        # The suite package is ``tests_<pkg>``, so the override reads
        # ``tests_<pkg>.*``. Match the PATTERN, not one literal, so the guard
        # keeps checking the policy instead of encoding a directory name.
        if isinstance(modules, list) and any(
            m.startswith("tests") and m.endswith(".*") for m in modules
        ):
            return override
    return None
