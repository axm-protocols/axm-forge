...
# The suite directory is ``tests_<pkg>`` (a unique module path per package —
# homonymous ``tests.conftest`` modules made the workspace suite uncollectable).
# Derive it from the package name so a rename cannot silently orphan the probe.
_SUITE = f"tests_{PACKAGE_ROOT.name.replace('-', '_')}"
TESTS_DIR = PACKAGE_ROOT / _SUITE / "integration"
