...
| `protocol_scaffolder.py` | `prepare_protocol_request()`, `preview_protocol_scaffold()` | Preview or apply the planner result. Application serializes preflight-through-rollback by canonical target root, while distinct roots remain concurrent; it also preflights confinement, symlinks and ownership before writing, then provides in-memory rollback for partial application failures |
