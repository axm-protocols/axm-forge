...
def test_name_and_category(strategy: CompactTablesStrategy) -> None:
...
def test_already_compact_table(strategy: CompactTablesStrategy) -> None:
    text = "|foo|bar|\n|---|---|\n|a|b|"
    ctx = SmeltContext(text=text, format=Format.MARKDOWN)
    result = strategy.apply(ctx)
...
def test_table_inside_fenced_code(strategy: CompactTablesStrategy) -> None:
    text = "```\n| foo | bar |\n| --- | --- |\n```"
    ctx = SmeltContext(text=text, format=Format.MARKDOWN)
    result = strategy.apply(ctx)
