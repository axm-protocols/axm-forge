...
3. Build a `SmeltContext` from the input text and detected format, then apply strategies in order — each `strategy.apply(ctx)` receives and returns a `SmeltContext`. If the output text differs, the strategy is recorded as applied
4. Count output tokens and compute `savings_pct`

`check()` runs every registered strategy independently on the original `SmeltContext` and records per-strategy savings without chaining.
...
sequenceDiagram
    ...
    loop For each strategy in preset
        ...
        Strategies-->>Pipeline: ctx (compacted or unchanged)
    end
