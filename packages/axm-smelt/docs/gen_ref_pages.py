...
It scans ``src/axm_smelt/`` for Python modules and generates a
``::: module.path`` page for each, which mkdocstrings then renders
under ``reference/axm_smelt/``. A ``SUMMARY.md`` is emitted for the
mkdocs-literate-nav plugin so the reference tree is navigable.
...
for path in sorted(src.rglob("*.py")):
    ...
    full_doc_path = Path("reference", *doc_parts).with_suffix(".md")
