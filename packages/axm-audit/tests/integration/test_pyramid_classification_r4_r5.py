...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            """,
            "pkg_dir",
            id="r4_conftest_fixture_with_write_text_detected",
        ),
        pytest.param(
            """
            import pytest
            ...
            """,
            "fixture_a",
            id="r4_transitive_fixture_dep",
        ),
    ],
)
def test_r4_conftest_fixture_io_detected(
    tmp_path: Path, conftest: str, test: str, fixture_name: str
) -> None:
    pkg = _make_pkg(tmp_path, test, conftest_source=conftest)
    finding = _find(pkg)
    assert finding.level == "integration"
    assert any(
        sig.startswith(f"conftest-fixture-io:{fixture_name}")
        for sig in finding.io_signals
    ), finding.io_signals
...
@pytest.mark.parametrize(
    ("test_source", "subdir", "expected_substring"),
    [
        pytest.param(
            """
            ...
            "unit",
            "mock-neutralized:",
            id="r5_soft_only_with_mock_httpx_becomes_unit",
        ),
        pytest.param(
            """
            from unittest.mock import patch
            ...
            "unit",
            "mock-neutralized",
            id="r5_patch_object_recognised_target",
        ),
        pytest.param(
            """
            import subprocess
            ...
            "unit",
            "mock-neutralized",
            id="r5_monkeypatch_setattr_recognised",
        ),
        pytest.param(
            """
            ...
            "unit",
            "mock-factory",
            id="r5_mock_factory_with_soft_signals_neutralizes",
        ),
    ],
)
def test_r5_neutralized_to_unit(
    tmp_path: Path, test_source: str, subdir: str, expected_substring: str
) -> None:
    pkg = _make_pkg(tmp_path, test_source, test_subdir=subdir)
    finding = _find(pkg)
    assert finding.level == "unit", (finding.level, finding.io_signals)
    assert any(expected_substring in sig for sig in finding.io_signals), (
        finding.io_signals
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            """,
            "integration",
            id="r5_hard_writer_with_mock_stays_integration",
        ),
        pytest.param(
            """
            import httpx
            ...
            """,
            "integration",
            id="r5_no_mock_at_all_stays_integration",
        ),
        pytest.param(
            """
            from unittest.mock import patch
            ...
            """,
            "integration",
            id="r5_target_not_matching_io_does_not_neutralize",
        ),
    ],
)
def test_r5_stays_integration(tmp_path: Path, test_source: str, subdir: str) -> None:
    pkg = _make_pkg(tmp_path, test_source, test_subdir=subdir)
    finding = _find(pkg)
