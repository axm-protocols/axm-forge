...
def test_safe_filename_substitutes_dash(inp: str, expected: str) -> None:
    """AC2: _safe_filename replaces legacy `-` separators, no-ops otherwise."""
...
def test_tier_for_path_finds_nested(path_str: str, expected: str | None) -> None:
    """AC2: _tier_for_path walks parts to find the tier component."""
    assert _tier_for_path(Path(path_str)) == expected


def test_retier_substitution_branch() -> None:
    """AC2: _retier substitutes the tier component for tests/<tier>/...rest."""
    root = Path("/p")
    src = Path("/p/tests/integration/test_X.py")
    assert _retier(src, root, "unit") == Path("/p/tests/unit/test_X.py")


def test_retier_inject_missing_tier() -> None:
    """AC2: _retier injects tier when path is tests/<file>.py (depth-2 corner case)."""
    root = Path("/p")
...
def test_retier_non_tests_unchanged() -> None:
    """AC2: _retier returns paths outside tests/ unchanged."""
    root = Path("/p")
    src = Path("/p/src/foo/bar.py")
    assert _retier(src, root, "unit") == Path("/p/src/foo/bar.py")


def test_module_path_dotted_and_outside(tmp_path: Path) -> None:
    """AC2: _module_path_for_test_file returns dotted path or None outside project."""
    project = tmp_path / "p"
    test_file = project / "tests" / "integration" / "test_foo.py"
    test_file.parent.mkdir(parents=True)
    test_file.write_text("")
    assert (
        _module_path_for_test_file(test_file, project) == "tests.integration.test_foo"
    )

    outside = tmp_path / "elsewhere" / "test_foo.py"
    outside.parent.mkdir(parents=True)
    outside.write_text("")
    assert _module_path_for_test_file(outside, project) is None


def test_abspath_normalises_relative_and_absolute() -> None:
    """AC2: _abspath joins relative paths to project, keeps absolute paths."""
    project = Path("/p")
    assert _abspath("tests/test_x.py", project) == project / "tests" / "test_x.py"
    assert _abspath("/abs/test_y.py", project) == Path("/abs/test_y.py")


def test_module_path_returns_none_when_outside_tests_dir(tmp_path: Path) -> None:
    """AC2: _module_path_for_test_file returns None for in-project non-tests paths."""
    project = tmp_path / "p"
    src_file = project / "src" / "axm_audit" / "mod.py"
    src_file.parent.mkdir(parents=True)
    src_file.write_text("")
    assert _module_path_for_test_file(src_file, project) is None


def test_file_depth_returns_zero_for_path_outside_project(tmp_path: Path) -> None:
    """AC2: _file_depth_from_project returns 0 when path is outside project_path."""
    project = tmp_path / "p"
    project.mkdir()
    outside = tmp_path / "elsewhere" / "test_x.py"
    outside.parent.mkdir()
    outside.write_text("")
    assert _file_depth_from_project(outside, project) == 0


def test_file_depth_invariant_under_project_path(tmp_path: Path) -> None:
    """AC2: _file_depth_from_project depends on relative path, not project depth."""
    ...
    f1.write_text("")
    f2.write_text("")
    assert _file_depth_from_project(f1, project1) == _file_depth_from_project(
        f2, project2
    )
